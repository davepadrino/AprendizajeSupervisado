...
parse_data = function(json){
  distance.text <- datos$rows$elements[[1]]$distance["text"][[1]]
  distance.value <- datos$rows$elements[[1]]$distance["value"][[1]]
  duration.text <- datos$rows$elements[[1]]$duration["text"][[1]]
  duration.value <- datos$rows$elements[[1]]$duration["value"][[1]]
  final.info <- c(distance.text, distance.value, duration.text, duration.value)  
  return(final.info)
